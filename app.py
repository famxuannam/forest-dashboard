...
try:
    _LOCAL_DEV_ADDRESS = (st.get_option("server.address") or "localhost").strip().lower()
except Exception:
    _LOCAL_DEV_ADDRESS = "localhost"
_IS_LOOPBACK_SERVER = _LOCAL_DEV_ADDRESS in {"localhost", "127.0.0.1", "::1"}
# Local dev dùng dữ liệu mẫu trong bộ nhớ, tuyệt đối không cần/không đọc Supabase secrets.
_USE_FAKE_DATA = _LOCAL_DEV_REQUESTED and _IS_LOOPBACK_SERVER
...
# cổng đăng nhập chỉ vì môi trường còn sót FOREST_LOCAL_DEV=1.
if _LOCAL_DEV_REQUESTED and not _IS_LOOPBACK_SERVER:
    st.error(
        "`FOREST_LOCAL_DEV` chỉ được phép khi server bind vào localhost/127.0.0.1/::1; "
        "không thể bỏ qua đăng nhập trên địa chỉ mạng công khai.")
    st.stop()
_skip_auth_for_local_dev = _USE_FAKE_DATA
...
if _auth_configured and not _skip_auth_for_local_dev:
...
if not _has_supabase_secrets and not _USE_FAKE_DATA:
    st.error(
...
def _body_font_b64(file_prefix, weights=None):
    ...
    weights=(400,600,800,...) (font tĩnh nhiều mức đậm, xem "Be Vietnam Pro") -> đọc
    `<file_prefix>-<weight>-<subset>.woff2`, trả {(weight,subset): b64}."""
    out = {}
    if weights:
        for w in weights:
            for name in ("latin", "latin-ext", "vietnamese"):
                with open(os.path.join("assets", "fonts", f"{file_prefix}-{w}-{name}.woff2"), "rb") as f:
                    out[(w, name)] = base64.b64encode(f.read()).decode()
    else:
        for name in ("latin", "latin-ext", "vietnamese"):
...
_body_font_weights = BODY_FONTS[BODY_FONT_NAME].get("weights")
_body_font_b64_cache = _body_font_b64(BODY_FONTS[BODY_FONT_NAME]["file_prefix"], _body_font_weights)
if _body_font_weights:
    _BODY_FONT_FACE = "".join(
        f"@font-face {{ font-family:'{BODY_FONT}'; font-style:normal; font-weight:{_w}; "
        f"font-display:swap; src:url(data:font/woff2;base64,{_body_font_b64_cache[(_w, _name)]}) format('woff2'); "
        f"unicode-range:{_ranges}; }}"
        for _w in _body_font_weights
...
_MAIN_CSS = """
    ...
    /* Luồng 2 cột today_flow2 (trang Hôm nay -- xem render_day_report()) -- mỗi chương (kể cả
       chương 1 với TOÀN BỘ nội dung con, không chia lẻ, xem docstring nơi gọi) là 1 khối
       break-inside:avoid, CSS tự cân bằng chiều cao 2 cột theo đúng thứ tự DOM. display:block
       !important BẮT BUỘC: [data-testid="stVerticalBlock"] mặc định display:flex (để dùng gap)
       qua rule chung, mà column-count KHÔNG có tác dụng trên flex container (xác nhận qua
       getComputedStyle -- columnCount báo "2" nhưng layout vẫn full-width vì display vẫn "flex",
       CSS multi-column chỉ hoạt động trên block/flow-root formatting context). MẶC ĐỊNH 1 cột
       (display:block để giữ nếp gấp nội dung dọc bình thường) -- 2 cột CHỈ bật ở màn >=1300px qua
       @media min-width bên dưới (xác nhận với người dùng: 640px quá hẹp, tablet dọc ~768px mỗi
       cột chưa tới 350px rất chật). */
    .st-key-today_flow2 { display: block !important; }
    .st-key-today_flow2 > [data-testid="stLayoutWrapper"] { margin-bottom: 14px; }
    /* MỌI tiêu đề chương trong luồng (kể cả chương 2-5 vốn KHÔNG truyền tight_top=True cho
       ...
       header "1" top=308px, header "3" top=316px, đúng 8px lệch). Khi 2 tiêu đề CÙNG là mục ĐẦU
       của 1 cột (bug thật đã gặp, xem ảnh chụp người dùng gửi) -- lệch 8px lộ rõ vì đứng cùng
       hàng. Đặt margin-top:0 ĐỒNG NHẤT cho mọi tiêu đề trong luồng, dựa hẳn vào margin-bottom:14px
       của .st-key-today_flow2 > [data-testid="stLayoutWrapper"] (rule ngay trên) để tạo khoảng
       cách GIỮA các khối thay vì margin-top của tiêu đề kế tiếp -- tránh cộng dồn 2 nguồn margin
       cho các chương KHÔNG phải mục đầu cột. */
    [class*="st-key-today_flow2_i"] .sec-ch { margin-top: 0 !important; }
    @media (min-width: 1300px) {
        .st-key-today_flow2 { column-count: 2; column-gap: 24px; }
        .st-key-today_flow2 > [data-testid="stLayoutWrapper"] { break-inside: avoid; }
        /* Khối chương 1 (today_flow2_i1) chiếm ĐÚNG 1/2 trang ở >=1300px -- ép các cặp card con
           ...
           chia tiếp thành 4 cột quá hẹp (phản hồi thực tế người dùng qua ảnh chụp). CHỈ áp dụng
           trong media query này -- dưới 1300px today_flow2 đã tự về 1 cột full-width nên chương 1
           lại có đủ chỗ, để 2 card đó tự chia đôi bình thường như mọi trang khác gọi 2 hàm này.
           flex-basis:auto (không phải 0) vì lý do đã ghi ở rule mobile .sp-lcols phía dưới --
           overflow:hidden + flex-basis:0 trên flex-direction:column làm thẻ co sập mất nội dung,
           xem docstring rule đó. */
        .st-key-today_flow2_i1 .sp-lcols { flex-direction: column; }
        .st-key-today_flow2_i1 .sp-lcols > .sp-listcard { flex: 1 1 auto; }
        /* stColumn (con của stHorizontalBlock) có sẵn flex: 1 1 calc(50% - 16px) từ class emotion
           riêng của Streamlit (không phải inline style) cho bố cục 2 cột NGANG gốc -- đổi
           flex-direction sang column KHÔNG tự đổi flex-basis đó, khiến mỗi thẻ dù đã xếp dọc vẫn
           chỉ rộng 50% (bug thật đã gặp, xem ảnh chụp người dùng gửi: "Theo buổi"/"Độ dài phiên"
           xếp dọc nhưng mỗi thẻ chỉ chiếm nửa cột). Ép flex-basis/width về 100% !important để mỗi
           thẻ full chiều ngang cột khi đã xếp dọc. */
        .st-key-today_flow2_i1 [data-testid="stHorizontalBlock"] { flex-direction: column; }
    ...
    @media (max-width: 640px) {
        ...
        /* 2 thẻ list (.sp-lcols) xếp dọc trên mobile thay vì cạnh nhau -- màn hẹp không đủ chỗ
           cho 2 cột mà vẫn đọc được các dòng nhãn/giá trị. flex-basis PHẢI đổi từ 0 (desktop,
           chia đều bề NGANG) sang auto ở đây: khi flex-direction đổi thành column, flex-basis:0
           lại tính theo chiều CAO, và .sp-listcard có overflow:hidden nên kích thước tối thiểu tự
           động theo nội dung bị VÔ HIỆU (đúng theo spec CSS flexbox: chỉ áp dụng khi overflow:
           visible) -- thẻ co sập gần 0px, cắt mất chữ bên trong (bug thật đã gặp, xem ảnh chụp
           người dùng gửi, verify bằng getBoundingClientRect() thấy .sp-lrow cao 68px nhưng
           .sp-listcard cha chỉ đo được 34px). flex-basis:auto để mỗi thẻ cao đúng theo nội dung. */
        .sp-lcols { flex-direction: column; gap: 10px; }
        .sp-lcols > .sp-listcard { flex: 1 1 auto; }
...
def render_day_report(df):
    ...
    if day_df.empty:
        ...
        # TOÀN BỘ nội dung sau billboard đổ chung vào 1 luồng CSS multi-column (2 cột ở màn rộng)
        # -- mỗi CHƯƠNG (kể cả chương 1 với toàn bộ nội dung con của nó, KHÔNG chia lẻ tiếp) là 1
        # khối break-inside:avoid trong CÙNG 1 st.container(key="today_flow2"), CSS tự chảy đầy
        # cột trái rồi mới sang cột phải THEO ĐÚNG THỨ TỰ DOM (không dùng st.columns() để chọn cột
        # bằng Python) -- lý do: nếu Python tự tính rồi gán cứng khối vào cột trái/phải, trên
        # mobile khi dồn về 1 cột sẽ hiện SAI thứ tự đọc gốc 1-2-3-4-5. CSS multi-column dùng 1 DOM
        # DUY NHẤT nên mobile tự đúng thứ tự (chỉ cần bỏ column-count ở breakpoint hẹp, xem rule
        # mobile bên dưới).
        #
        # Chương 1 GIỮ NGUYÊN 1 khối duy nhất (tiêu đề + hero + stat listcard + dòng thời gian +
        # theo buổi/độ dài phiên) thay vì tách rời từng mảnh trộn chung với chương 2-5 (bản trước
        # đó) -- xác nhận với người dùng qua ảnh chụp: tách rời khiến tiêu đề chương 1 (dùng
        # chung, full-width) và tiêu đề chương 3 (khởi đầu cột phải) LỆCH NHAU (1 ở full-width bên
        # trên, 1 ở giữa cột) thay vì cùng hàng. Coi chương 1 là 1 khối atomic như chương 2-5 thì
        # tiêu đề của khối ĐẦU cột trái và khối ĐẦU cột phải tự nhiên cùng hàng, rule kẻ ngang tự
        # cắt đúng 1/2 màn hình (vì tiêu đề giờ nằm TRONG luồng cột, không còn full-width riêng).
        #
        # Đồng thời KHÔNG chia đôi thêm bất kỳ card con nào bên trong 1 khối nữa (stat listcard
        # "vs Thứ Tư.../Phiên đầu..." và cặp "Theo buổi"/"Độ dài phiên" bình thường tự chia 2 cột
        # khi đủ rộng -- ở đây mỗi khối chỉ chiếm ĐÚNG 1/2 trang nên chia tiếp sẽ ra 4 cột quá hẹp,
        # xác nhận với người dùng) -- ép về 1 cột qua CSS scope riêng .st-key-today_flow2_i1 (xem
        # rule bên dưới), không đổi hành vi mặc định của render_stat_panel()/render_project_rhythm()
        # ở các trang khác.
        with st.container(key="today_flow2"):
            with st.container(key="today_flow2_i1"):
                ...
                # Dòng thời gian đứng NGAY SAU stat panel -- theo đúng bố cục "Sổ Tay": nhìn được
                # nhịp phiên trong ngày trước khi đọc số liệu tổng hợp bên dưới. Bỏ lớp mờ "khung
                # giờ điển hình của thứ này" (không cần thiết, gây rối) -- chỉ còn khối phiên +
                # legend theo Dự án. KHÔNG có Top 3 Nhóm/Dự án ở đây (khác Báo cáo theo kỳ) -- 1
                # ngày thường chỉ 2-4 phiên, đã thấy rõ hết trong dòng thời gian ngay phía trên,
                # xếp hạng top 3 chỉ lặp lại thông tin.
                render_day_timeline(day_df)

                render_project_rhythm(day_df)

            with st.container(key="today_flow2_i4"):
                sec_chapter("today-ch2", 2, "Phân bổ thời gian")
                frag_category_bars(day_df, "rad_day", "Dự án")

            with st.container(key="today_flow2_i5"):
                sec_chapter("today-ch3", 3, "Ghi chú ngày")
                render_note_editor(sel, sel_day_badges)

            with st.container(key="today_flow2_i6"):
...
""", unsafe_allow_html=True)
            ...
            with st.container(key="today_flow2_i7"):
                sec_chapter("today-ch5", 5, "Ngày này năm trước")
                render_on_this_day(sel, df)


def _tb_axis_grid(items, per_row, is_selected, setting_name, key_prefix, css_for,
                   label_for=None, value_for=None):
    """Lưới nút chọn 1 trục cá nhân hoá (sub-page "Giao diện") -- items: list [(name, cfg)].
    is_selected(name, cfg) -> bool. css_for(name, cfg, selected) -> chuỗi thuộc tính CSS (không
    kèm selector) áp cho nút preview của item đó. value_for(name, cfg) -> giá trị THẬT SỰ lưu vào
    setting (mặc định chính tên item -- đúng cho palette/kiểu nền trang/kiểu thẻ/mật độ/font, vốn
    lưu bằng tên; accent là NGOẠI LỆ DUY NHẤT lưu bằng mã hex nên truyền value_for=lambda n, h: h).
    Bấm 1 item chưa chọn -> save_setting() + st.rerun(), đúng pattern nút-preview đã dùng cho
    accent/bảng màu nền/kiểu nền trang/kiểu thẻ/font từ trước -- gộp lại thành 1 hàm dùng chung
    cho cả 6 trục thay vì lặp lại y hệt 6 lần."""
    _label_for = label_for or (lambda n: n)
    _value_for = value_for or (lambda n, c: n)
    _css = "<style>"
    for _start in range(0, len(items), per_row):
        _row = items[_start:_start + per_row]
        _cols = st.columns(per_row)
        for _i, (_name, _cfg) in enumerate(_row):
            _idx = _start + _i
            _key = f"{key_prefix}_{_idx}"
            _selected = is_selected(_name, _cfg)
            _label = f"✓ {_label_for(_name)}" if _selected else _label_for(_name)
            _css += (f".st-key-{_key} div[data-testid=\"stButton\"] button[kind=\"secondary\"] {{ "
                     f"{css_for(_name, _cfg, _selected)} }}")
            with _cols[_i]:
                if st.button(_label, key=_key, use_container_width=True):
                    if not _selected:
                        save_setting(setting_name, _value_for(_name, _cfg))
                        st.rerun()
    _css += "</style>"
...
def _render_tuybien_giao_dien():
    ...
    _right_html = (
        "<div class='pbill-title'>Xem trước trực tiếp</div>"
        "<div class='pbill-sub'>Tổ hợp đang áp dụng — cập nhật ngay khi bạn đổi 1 lựa chọn "
        "bất kỳ bên dưới.</div>"
    ...
    with _reset_col:
        if st.button("Đặt lại mặc định", key="tbgd_reset_all", use_container_width=True):
            ...
            st.rerun()
    with _random_col:
        if st.button("Ngẫu nhiên", icon=":material/casino:", key="tbgd_randomize", use_container_width=True):
    ...
    with st.container(border=True, key="tbgd_accent_card"):
        def _accent_css(name, hexcode, selected):
            _border = "var(--text)" if selected else "transparent"
            _txt = _readable_text(hexcode)
            return (f"background:{hexcode} !important; color:{_txt} !important; "
    ...
    with st.container(border=True, key="tbgd_palette_card"):
        def _pal_css(name, tok, selected):
            _bg = tok["bg"][1] if IS_DARK else tok["bg"][0]
            _card = tok["card"][1] if IS_DARK else tok["card"][0]
    ...
    with st.container(border=True, key="tbgd_pattern_card"):
        def _bgpat_css(name, cfg, selected):
            ...
            _position = cfg.get("position", "0 0")
            return (f"background-color: var(--card-tl) !important; "
                    f"background-image: {cfg['image']} !important; background-size: {cfg['size']} !important; "
                    f"background-position: {_position} !important; "
                    f"color: var(--text) !important; border:2px solid {_border} !important; "
                    f"border-radius:10px !important; width:100% !important; height:auto !important; "
    ...
    with st.container(border=True, key="tbgd_cardstyle_card"):
        def _cs_css(name, cfg, selected):
            ...
            return (f"background-color: var(--card) !important; color: var(--text) !important; "
                    f"border:{cfg['border_w']} solid {_ring} !important; "
                    f"border-radius:{cfg['radius']} !important; box-shadow:{cfg['shadow']} !important; "
                    f"width:100% !important; height:auto !important; min-height:48px !important; "
                    f"padding:8px 6px !important; font-weight:600 !important; font-size:13px !important; "
                    f"white-space:normal !important; line-height:1.25 !important;")
    ...
    with st.container(border=True, key="tbgd_font_card"):
        def _bf_css(name, cfg, selected):
            _ring = "var(--accent)" if selected else "var(--border)"
            return (f"background-color: var(--card) !important; color: var(--text) !important; "
                    f"border:var(--card-border-w) solid {_ring} !important; "
                    f"border-radius:var(--card-radius) !important; "
                    f"font-family:'{cfg['family']}',-apple-system,BlinkMacSystemFont,\"Segoe UI\","
...
# ==========================================
    ...
    if tb_sub == "Giao diện":
        ...
        # Hero + 4 chương (chuyển từ 5 expander đánh số cũ, xác nhận với người dùng giữ NGUYÊN mọi
        # luồng nạp dữ liệu/xử lý bên trong, chỉ đổi vỏ ngoài theo mockup Forest Dashboard.dc.html).
        # Chương "Giao diện" (trước đây là chương 3 ở đây) đã tách thành sub-page riêng ở trên theo
        # mockup "Tuỳ Chỉnh Giao Diện.dc.html" -- xem _render_tuybien_giao_dien(). Chương "Dữ liệu
        # làm việc hiện tại" (bảng phiên thô + xoá hàng loạt) KHÔNG có trong mockup gốc -- xác nhận
        # với người dùng giữ làm chương riêng cuối cùng, đặt SAU "Quản lý hệ thống".
        ...
        render_period_billboard(
            ...
            "<div class='pbill-sub'>Nạp dữ liệu, gán phân loại, quản lý hệ thống và sao lưu — "
