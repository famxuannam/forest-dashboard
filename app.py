...
# Bảng màu Catppuccin Latte (Light Theme)
LATTE_COLORS = [
    "#8839ef", # Mauve
    "#1e66f5", # Blue
    "#40a02b", # Green
...
def add_total_labels(fig, df, x_col, y_col):
    """Hàm dán nhãn tổng cộng lên đỉnh của cột biểu đồ xếp chồng"""
    totals = df.groupby(x_col)[y_col].sum().reset_index()
    fig.add_trace(go.Scatter(
        x=totals[x_col],
        y=totals[y_col],
        mode='text',
        text=totals[y_col].round(1).astype(str),
        textposition='top center',
        showlegend=False,
        hoverinfo='skip',
        textfont=dict(color="#4c4f69", size=13)
    ))
    fig.update_layout(yaxis=dict(range=[0, totals[y_col].max() * 1.15]))
    return fig

# --- KHỞI TẠO STATE CHO ĐIỀU HƯỚNG ---
if 'sel_month' not in st.session_state: st.session_state.sel_month = None
...
st.markdown(
    ...
    /* Ép độ rộng của nút bấm cho đồng đều */
...
with tabs[0]:
    ...
    if forest_file:
        if st.button("Xác nhận cập nhật dữ liệu", type="primary"):
            ...
            if 'Is Success' in new_data.columns:
                new_data = new_data[new_data['Is Success'] == True]
    ...
    with c1:
        ...
        if os.path.exists(DB_FILE):
            with open(DB_FILE, "rb") as f:
                st.download_button("Tải file Dữ liệu", f, "database.csv", "text/csv")
        if os.path.exists(MAPPING_FILE):
            with open(MAPPING_FILE, "rb") as f:
                st.download_button("Tải file Quy tắc", f, "mapping.csv", "text/csv")
    with c2:
...
with tabs[1]:
    if not df.empty:
        ...
        pivot_df = get_pivot_with_totals(df, time_col)
        st.dataframe(pivot_df, width=CHART_WIDTH)
    else:
...
with tabs[2]:
    if not df.empty:
        ...
        if st.session_state.sel_month not in months:
            st.session_state.sel_month = months[-1]
            
        c_prev, c_sel, c_next = st.columns([1, 2, 1])
        curr_m_idx = months.index(st.session_state.sel_month)
        
        with c_prev:
            if st.button("◀ Trước", key="btn_prev_m") and curr_m_idx > 0:
                st.session_state.sel_month = months[curr_m_idx - 1]
                st.rerun()
        with c_sel:
            # Ẩn hoàn toàn label để thanh selectbox nằm ngang hàng với 2 nút bấm
            selected_month = st.selectbox("Chọn Tháng", months, index=curr_m_idx, key="month_box", label_visibility="collapsed")
            if selected_month != st.session_state.sel_month:
                st.session_state.sel_month = selected_month
                st.rerun()
        with c_next:
            if st.button("Sau ▶", key="btn_next_m") and curr_m_idx < len(months) - 1:
                st.session_state.sel_month = months[curr_m_idx + 1]
                st.rerun()

        df_m = df[df['Tháng'] == st.session_state.sel_month]
        ...
        if not df_m.empty:
            ...
            fig_m = add_total_labels(fig_m, t_m, 'Ngày', 'Số giờ') # Thêm nhãn tổng
            ...
            st.plotly_chart(fig_p_m, use_container_width=False)
            
            st.markdown("**Bảng chi tiết (Giờ):**")
            pt_m = get_table_with_totals(df_m)
            st.dataframe(pt_m, width=CHART_WIDTH)
...
with tabs[3]:
    if not df.empty:
        ...
        if st.session_state.sel_week not in weeks:
            st.session_state.sel_week = weeks[-1]
            
        c_prev_w, c_sel_w, c_next_w = st.columns([1, 2, 1])
        curr_w_idx = weeks.index(st.session_state.sel_week)
        
        with c_prev_w:
            if st.button("◀ Trước", key="btn_prev_w") and curr_w_idx > 0:
                st.session_state.sel_week = weeks[curr_w_idx - 1]
                st.rerun()
        with c_sel_w:
            selected_week = st.selectbox("Chọn Tuần", weeks, index=curr_w_idx, key="week_box", label_visibility="collapsed")
            if selected_week != st.session_state.sel_week:
                st.session_state.sel_week = selected_week
                st.rerun()
        with c_next_w:
            if st.button("Sau ▶", key="btn_next_w") and curr_w_idx < len(weeks) - 1:
                st.session_state.sel_week = weeks[curr_w_idx + 1]
                st.rerun()

        df_w = df[df['Tuần'] == st.session_state.sel_week]
        ...
        if not df_w.empty:
            ...
            fig_w = add_total_labels(fig_w, t_w, 'Thứ', 'Số giờ') # Thêm nhãn tổng
            st.plotly_chart(fig_w, use_container_width=False)
            ...
            st.plotly_chart(fig_p_w, use_container_width=False)
            
            st.markdown("**Bảng chi tiết (Giờ):**")
            pt_w = get_table_with_totals(df_w)
            st.dataframe(pt_w, width=CHART_WIDTH)
            ...
            st.plotly_chart(fig_hw, use_container_width=False)
...
with tabs[4]:
    if not df.empty:
        ...
        # Biểu đồ tự động đưa vào khung trắng do CSS ở đầu file
        chart = alt.Chart(cal_data).mark_rect(cornerRadius=3).encode(
            ...
            color=alt.Color('Số giờ:Q', 
                            scale=alt.Scale(domain=[0, cal_data['Số giờ'].max() if cal_data['Số giờ'].max() > 0 else 1], 
                                            range=['#e6e9ef', '#40a02b']), 
                            legend=None),
            tooltip=[alt.Tooltip('Ngày_str:T', format='%Y-%m-%d', title='Ngày'), alt.Tooltip('Số giờ:Q', title='Thời lượng (Giờ)')]
        ).properties(
            width=alt.Step(40),  
            height=alt.Step(40)  
        ).configure_view(
        ...
        if not unique_dates.empty:
            ...
            if (db_max_date - proj_max_date).days <= 1:
                current_streak = streak_counts[streak_id.iloc[-1]]
            else:
                current_streak = 0
        else:
            total_days = longest_streak = current_streak = 0

        # Bảng thống kê chuẩn căn giữa, nằm trong hộp trắng bo góc
        st.markdown(f"""
