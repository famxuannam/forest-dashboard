...
- [Lưu trữ dữ liệu & Deploy](#lưu-trữ-dữ-liệu--deploy)
...
> 💡 Mặc định (chạy local, chưa cấu hình Supabase) dữ liệu được lưu ở thư mục chạy ứng dụng
> trong 4 file: `database.csv` (các phiên), `mapping.csv` (phân loại Dự án → Nhóm),
> `deleted.csv` (danh sách phiên đã xoá) và `notes.csv` (ghi chú theo ngày). Một bản sao lưu
> `.zip` đóng gói cả 4 phần này. Muốn deploy app lên internet (dùng từ nhiều máy/điện thoại
> qua cùng một URL), xem mục [Lưu trữ dữ liệu & Deploy](#lưu-trữ-dữ-liệu--deploy) bên dưới —
> local lưu file tạm thời trên nhiều nền tảng deploy free, cần chuyển sang Supabase để dữ
> liệu không mất khi container khởi động lại.
...
**supabase-py** (chỉ dùng khi cấu hình lưu trữ Supabase, xem mục dưới). Cấu hình giao diện
(màu nền, màu nhấn, font) nằm ở `.streamlit/config.toml`.

---

## Lưu trữ dữ liệu & Deploy

App hỗ trợ **2 chế độ lưu trữ**, tự động chọn theo cấu hình — không cần đổi gì trong code:

- **CSV cục bộ (mặc định)** — như mô tả ở trên, phù hợp khi chạy trên máy riêng (`streamlit
  run app.py`). Không cần cấu hình gì thêm.
- **Supabase (Postgres)** — cần khi **deploy app lên internet**. Hầu hết nền tảng deploy miễn
  phí (kể cả Streamlit Community Cloud) dùng ổ đĩa **tạm thời**: mọi file CSV sẽ **mất khi
  container khởi động lại** (redeploy, ngủ đông do không hoạt động…). Supabase là Postgres
  lưu ngoài, bền vững qua các lần khởi động lại, xem được từ mọi thiết bị qua cùng một URL.

App tự nhận biết chế độ: có cấu hình `SUPABASE_URL`/`SUPABASE_KEY` trong `st.secrets` thì dùng
Supabase, không thì dùng lại CSV cục bộ như cũ — không có cấu hình cũng không lỗi gì.

### Thiết lập Supabase (một lần)

1. Tạo tài khoản + project miễn phí tại [supabase.com](https://supabase.com).
2. Vào **SQL Editor** của project, chạy toàn bộ nội dung file [`supabase_schema.sql`](supabase_schema.sql)
   (tạo 4 bảng tương ứng 4 file CSV, cộng RLS mở vì app không có đăng nhập).
3. Vào **Settings → API**, lấy **Project URL** và khoá **anon public**.
4. Sao chép `.streamlit/secrets.toml.example` thành `.streamlit/secrets.toml`, điền 2 giá trị
   trên vào `SUPABASE_URL` / `SUPABASE_KEY` (file này đã có trong `.gitignore`, không commit).
5. Chạy lại `streamlit run app.py` — dữ liệu giờ đọc/ghi qua Supabase.

> ⚠️ Vì đã chọn **không có lớp đăng nhập**, ai có URL app cũng xem/sửa được dữ liệu — chỉ nên
> deploy với URL không công khai/không chia sẻ rộng rãi. Project Supabase free tier cũng tự
> "ngủ" (pause) sau khoảng 7 ngày không có hoạt động API; mở app lên gặp lỗi kết nối thì vào
> dashboard Supabase bấm un-pause (không mất dữ liệu, mất khoảng 1 phút).
...
3. Ở phần **Secrets** của app, dán `SUPABASE_URL` và `SUPABASE_KEY` (nội dung giống file
   `secrets.toml` ở bước thiết lập Supabase). Streamlit Cloud tự cài `requirements.txt`,
